...
#data, ref = cleanup_data(r"Logistic_regression\binary_classification_train.csv")
#data1 = data[0:48000, 1:]
##Logistic_regression_main(data1, np.zeros((1,np.size(data1, axis=1)-1)), 0)
#
#weights = load_data(r"Logistic_regression\weights.csv")
#b = load_data(r"Logistic_regression\bias.csv")[0][0]
#y = data[:, -1]
#x = data[:,1:-1]
#accuracy_measure(x, y, weights, b, 1000,0.6)

"""
data = load_data(r"Dataunsupervised_data.csv", 48000)
k = 3
K = 4
fig, axis = plt.subplots(2,k)
centroids, idx = K_means_clustering(K, data[:, 1:], 3, axis, 100, plot =False)
print(centroids)
"""
...
epochs = 3000
print("Loading data....")
#dataI, ref, means, sD  = cleanup_data(r"Data\multi_classification_train.csv", 22, batch_size, batch_size, normalization_factor=1, one_hot_encoding_column= 21)
dataI = np.array(load_data(r"Data\multi_classification_train.csv", "all", 0))
dataB = np.array(load_data(r"multi_classification_test_predicted_train.csv", "all", 0))
correct = dataB[:,-1] == dataI[:,-1]
print(correct)

print("Loaded data....")
#data_cv = dataI[int(batch_size-batch_size*0.1):,:]
#xtt = data_cv[:, 1:-5]
#ytt = data_cv[:, -5:]
#data = dataI[:int(batch_size-batch_size*0.1),:]
#xt  = data[:, 1:-5]
#yt = data[:, -5:]

r"""
data_test = normalize_data(r"Data\multi_classification_train.csv", means, sD, 1)[:,1:]
with open(r"multiclass_NN", "rb") as f:
    NN1 = pickle.load(f)
pred = np.zeros((data_test.shape[0], 5))
softmax = True
for record_number in range(pred.shape[0]):
    prediction = NN1.predict(np.array([data_test[record_number,:]]).T).T
    if softmax:
        max_index = np.argmax(prediction)
        prediction_s = np.zeros(prediction.shape)
        prediction_s[0, max_index] = 1
...
for i in range(pred.shape[0]):
    predicted_class.append(ref[-1][np.argmax(pred[i])])
print(pred, predicted_class)
df = pd.DataFrame(predicted_class)
df.to_csv(r"multi_classification_test_predicted_train.csv", index=False)
"""

#NN1 = Neural_network([Neural_layer(np.random.rand(10,20)/1000, np.random.rand(10,1)/1000, ReLU_activation, learning_rate=1e-3, optimization="Adam"),
#                      Neural_layer(np.random.rand(5,10)/1000, np.random.rand(5,1)/1000, sigmoid_activation, learning_rate=1e-3, optimization="Adam")], r"multiclass_NN")
#NN1.fit(mini_batch_size, xt, xtt, yt, ytt, epochs, 50, plotting_interval=1, cost_divergence_check=False, verbose=True)
#NN1.save()

r"""
with open(r"Neural_Networks\Neural_network_multiclass_test2", "rb") as f:
    NN1 = pickle.load(f)
NN1.cost_history = []
NN1.test_history = []
NN1.accuracy_cv = []
NN1.accuracy_train = []
#NN1 = Neural_network([Neural_layer(np.random.rand(25,1024)/1000, np.random.rand(25,1)/1000, ReLU_activation, learning_rate= 1e-3, optimization="Adam"), 
#                    Neural_layer(np.random.rand(25,25)/1000, np.random.rand(25,1)/1000, ReLU_activation, learning_rate=1e-3, optimization="Adam"),
#                    Neural_layer(np.random.rand(10,25)/1000, np.random.rand(10,1)/1000, softmax_activation, learning_rate=1e-3, optimization="Adam")], r"Neural_Networks\Neural_network_multiclass_test2")
NN1.fit(mini_batch_size, xt, xtt, yt, ytt, epochs, 100, plotting_interval=1, cost_divergence_check=False, verbose=True)
NN1.save()
NN1.summary(True)
"""
...
r"""
rows = 48000
data, ref = cleanup_data(r"Data\polynomial_regression_train.csv", 7, rows, rows-10000, 1)
data_all, ref =  cleanup_data(r"Data\polynomial_regression_train.csv", 7, 48000, 48000, 1)
data_cv = data_all[47000:,1:]
#w = load_data(r"Linear_regression\weights.csv", "all", None)
#b= load_data(r"Linear_regression\bias.csv", "all", None)[0][0]
data_train = data[:rows//2, 1:]
data_test = data[rows//2:rows-10000, 1:]
w = np.full((1, (np.size(data_train, axis=1)-1)*3),1)
b = 0
Linear_regression_main(data_train,data_test, data_cv, w, b,3)
"""
